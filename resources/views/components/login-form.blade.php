<form method="POST" action="{{ route('login') }}" class="space-y-4">
    ...
    {{-- Email --}}
    <div class="fromGroup">
        <label for="email" class="block capitalize form-label">{!! __('Correo electr&oacute;nico') !!}</label>
        <div class="relative ">
            <input type="email" name="email" id="email"
                class="form-control py-2 @error('email') !border !border-red-500 @enderror"
                placeholder="{{ __('Escriba su email') }}" autofocus value="{{ old('email') }}">
    ...
    <div class="fromGroup">
        ...
        <div class="relative ">
            <input type="password" name="password"
                ...
                placeholder="Escriba su {{ strtolower(__('Password')) }}" id="password"
    ...
    <div class="flex justify-between">
        ...
        <a href="{{ route('password.request') }}"
            ...
            {!! __('¿Olvidó su contrase&ntilde;a?') !!}
        </a>
    </div>

    <button type="submit" class="btn btn-dark block w-full text-center">
        {!! __('Iniciar sessi&oacute;n') !!}
    </button>
</form>
