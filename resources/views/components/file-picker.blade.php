...
<div class="flex w-full mt-4 items-center justify-between gap-2 border-b pb-2 md:pb-4 file-picker-container">
    ...
    <div class="flex items-center space-x-2">
        ...
        @if ($key === 'open')
            {{-- Badge azul “Abierto” --}}
            <span
                class="inline-flex items-center space-x-1 bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-100 px-2 py-1 rounded-full text-sm font-medium">
            ...
            @hasrole('client')
                ...
                <label for="{{ $fileId }}"
                    ...
                    <input type="file" accept=".jpg,.png,.xlsx,.xls,.csv,.doc,.docx,.ppt,.pptx,.pdf"
                        name="{{ $fileName }}" id="{{ $fileId }}" class="hidden"
        ...
        @elseif($key === 'waiting_review')
            @hasrole('client')
                ...
                <span
                    class="inline-flex items-center space-x-1 bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-100 px-2 py-1 rounded-full text-sm font-medium">
            ...
            @else
                ...
                <div class="flex gap-2">
                    <div class="hidden">
                        <form action="{{ route('documents.reject', ['document' => $document]) }}" method="GET"
                            class="inline confirm-form" data-action="reject">
                            @csrf
                            <button type="submit"
                                class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer hidden"
                                title="Rechazar" style="background: transparent; border: none;">
                                <iconify-icon icon="heroicons-solid:x-circle hidden"
                    ...
                    <form action="{{ route('documents.mark-as-accept', ['document' => $document]) }}" method="GET"
                        class="inline confirm-form" data-action="accept">
                        @csrf
                        <button type="submit"
                            class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition cursor-pointer"
                            title="Aceptar" style="background: transparent; border: none;">
                            <iconify-icon icon="heroicons-solid:check-circle"
                                class="text-2xl text-green-500"></iconify-icon>
                        </button>
                    </form>
                </div>
            @endhasrole
        ...
        @elseif($key === 'accepted')
            <span
                class="inline-flex items-center space-x-1 bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-100 px-2 py-1 rounded-full text-sm font-medium">
        ...
        @elseif($key === 'rejected')
            <span
                class="inline-flex items-center space-x-1 bg-red-100 dark:bg-red-800 text-red-800 dark:text-red-100 px-2 py-1 rounded-full text-sm font-medium">
    ...
    @if ($key !== 'open')
        <a href="{{ route('documents.download', ['document' => $document]) }}"
            class="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition" title="Descargar">
            <iconify-icon icon="heroicons-outline:download"
                class="text-xl text-slate-600 dark:text-slate-300"></iconify-icon>
        </a>
    @endif
</div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            forms.forEach(form => {
                form.addEventListener('submit', function(event) {
                    ...
                    event.stopPropagation(); // <-- Añade esto
